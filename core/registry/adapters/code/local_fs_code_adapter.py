...
class LocalFSCodeAdapter(BaseRegistryAdapter):
    '''
    adapter for local filesystem model code directory
    ...
    def fetch_model(self, model_id: str) -> Dict[str, Any]:
        '''
        fetch model code from local filesystem
    ...
    def download_model(self, model_id: str, destination: str) -> str:
        '''
        copy model code from local filesystem to destination
        '''
        model_dir = self.base_path / model_id
        if not model_dir.exists():
            raise ValueError(f"model directory not found: {model_id}")
        ...
        logger.info(f"copied model code {model_id} to {destination}")
        return destination
