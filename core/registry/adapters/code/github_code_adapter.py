...
class GitHubCodeAdapter(BaseRegistryAdapter):
    '''
    adapter for github repositories containing model code
    '''
    ...
    def download_model(self, model_id: str, destination: str) -> str:
        '''
        clone or download model code from github
        '''
        import subprocess
        import shutil
        try:
            ...
            logger.info(f"downloaded model code from {repo_url} to {destination}")
            return destination
        except Exception as e:
            logger.error(f"error downloading model code from github: {e}")
            raise
